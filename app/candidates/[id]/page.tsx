...
export default async function CandidateDetail({params}:{params:Promise<{id:string}>}){
 ...
 return <>
  <div className="card"><div className="sectionHeader"><div><div className="eyebrow">CANDIDAT</div><h1>{candidate.fullName}</h1><p className="muted">{candidate.location||"Localisation à confirmer"} · {candidate.email||"E-mail non détecté"} · {candidate.experienceYears?`${candidate.experienceYears} ans d'expérience détectés`:"Expérience à confirmer"}</p></div><Link className="btn secondary" href={`/search?q=${encodeURIComponent(candidate.fullName)}`}>Retrouver dans le vivier</Link></div>
