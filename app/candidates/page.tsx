...
export default async function CandidatesPage(){
 const candidates=await prisma.candidate.findMany({orderBy:{createdAt:"desc"},include:{matches:{orderBy:{score:"desc"},take:1,include:{job:true}}}});
 return <div className="card"><div className="sectionHeader"><div><div className="eyebrow">VIVIER</div><h1>Vivier candidats</h1><p className="muted">{candidates.length} candidat(s) enregistré(s).</p></div><div className="actions"><Link className="btn secondary" href="/search">Rechercher</Link><a className="btn secondary" href="/api/export/candidates">Exporter CSV</a><Link className="btn" href="/candidates/new">Importer un CV</Link></div></div>
