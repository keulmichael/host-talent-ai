...
export default async function ClientPack({params}:{params:Promise<{id:string}>}){
 ...
 if(!job)notFound();
 return <>
  <div className="card noPrint"><div className="sectionHeader"><div><div className="eyebrow">V1.9 · DOSSIER CLIENT</div><h1>Short-list client</h1><p className="muted">Présentation exploitable en rendez-vous ou en PDF, sans coordonnées privées ni notes internes.</p></div><div className="actions"><Link className="btn secondary" href={`/jobs/${job.id}`}>Retour mission</Link><Link className="btn secondary" href={`/jobs/${job.id}/compare`}>Comparer</Link><Link className="btn secondary" href={`/jobs/${job.id}/shares`}>Partager au client</Link><PrintButton/></div></div></div>
