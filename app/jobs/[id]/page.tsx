...
export default async function JobPage({params}:{params:Promise<{id:string}>}){
 ...
 return <>
  <div className="card"><div className="eyebrow">MISSION · V1.9</div><h1>{job.title}</h1><p className="muted">{job.clientName||"Client non renseigné"} · {job.location||"Localisation non renseignée"}</p><p>{job.description}</p>
  ...
  <div className="card" style={{marginTop:16}}><div className="sectionHeader"><div><h2>Profils du vivier</h2><p className="muted">{job.matches.length} profil(s) analysé(s) sur {candidateCount} candidat(s). {shortlisted} profil(s) retenu(s) dans la short-list client.</p></div></div>
   {candidateCount===0?<p className="muted">Importe d'abord des CV.</p>:job.matches.length===0?<p className="muted">Lance l'analyse du vivier.</p>:job.matches.map(m=><div className="matchRow" key={m.id}>
    ...
    <details className="opsDetails"><summary>Suivi recruteur</summary><MatchActions id={m.id} stage={m.stage} recruiterNote={m.recruiterNote} nextAction={m.nextAction}/><CandidateCommercial candidateId={m.candidate.id} availability={m.candidate.availability} dailyRate={m.candidate.dailyRate} salaryExpectation={m.candidate.salaryExpectation}/></details>
   </div>)}
  </div>
 </>;
}
