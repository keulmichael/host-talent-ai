...
export default async function ComparePage({params}:{params:Promise<{id:string}>}){
 ...
 return <div className="card">
  <div className="sectionHeader"><div><div className="eyebrow">V1.9 · COMPARATEUR</div><h1>Comparer les candidats</h1><p className="muted">{job.title} · jusqu'aux 5 meilleurs profils du vivier.</p></div><div className="actions"><Link className="btn secondary" href={`/jobs/${job.id}`}>Retour mission</Link><Link className="btn" href={`/jobs/${job.id}/client`}>Dossier client</Link><Link className="btn secondary" href={`/jobs/${job.id}/shares`}>Portail client</Link></div></div>
  {job.matches.length===0?<p className="muted">Aucun matching calculé.</p>:<div className="tableWrap"><table><thead><tr><th>Critère</th>{job.matches.map(m=><th key={m.id}><Link href={`/candidates/${m.candidateId}`}>{m.candidate.fullName}</Link></th>)}</tr></thead><tbody>
   ...
   <tr><td><strong>Disponibilité</strong></td>{job.matches.map(m=><td key={m.id}>{m.candidate.availability||"À confirmer"}</td>)}</tr>
   <tr><td><strong>TJM</strong></td>{job.matches.map(m=><td key={m.id}>{m.candidate.dailyRate?`${m.candidate.dailyRate} € / jour`:"—"}</td>)}</tr>
   <tr><td><strong>Prétention salariale</strong></td>{job.matches.map(m=><td key={m.id}>{m.candidate.salaryExpectation?`${m.candidate.salaryExpectation.toLocaleString("fr-FR")} € / an`:"—"}</td>)}</tr>
  ...
  <p className="muted small" style={{marginTop:18}}>Le comparateur met les éléments du CV et les données commerciales côte à côte. Il ne remplace pas la validation humaine du recruteur.</p>
 </div>;
}
