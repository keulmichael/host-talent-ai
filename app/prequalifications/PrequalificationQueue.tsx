...
export default function PrequalificationQueue({items}:{items:Item[]}){
 ...
 async function generate(ids=selected){if(!ids.length)return;setBusy(true);setMessage("");const r=await fetch("/api/prequalifications/links",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({candidateIds:ids})});const j=await r.json();setBusy(false);if(!r.ok){setMessage(j.error||"Impossible de générer les liens");return;}const text=(j.links||[]).map((x:any)=>`${x.fullName} — ${x.url}`).join("\n");await navigator.clipboard.writeText(text);setMessage(`${j.links.length} lien(s) copié(s) dans le presse-papiers.`);setSelected([]);router.refresh();}
 const label=(s:Item["status"])=>s==="COMPLETED"?"Complété":s==="LINK_CREATED"?"Lien généré":"À solliciter";
 return <>
  <div className="card sectionCard"><div className="sectionHeader"><div><h2>File de préqualification</h2><p className="muted">Sélectionnez plusieurs candidats pour générer leurs liens en une seule opération. Le statut évolue automatiquement lorsque le candidat complète son formulaire.</p></div><div className="actions"><button className="btn secondary" disabled={!selected.length||busy} onClick={()=>generate()}>{busy?"Génération…":`Générer ${selected.length||""} lien(s)`}</button></div></div>{message&&<p><strong>{message}</strong></p>}
   ...
   {items.map(i=><div className="reviewQueueRow" key={i.id}>
    ...
    <div className="reviewQueueMain"><small>Disponibilité : {i.availability||"—"}</small><small>TJM : {i.dailyRate?`${i.dailyRate} €":"—"}</small><small>Salaire : {i.salaryExpectation?`${i.salaryExpectation.toLocaleString("fr-FR")} €":"—"}</small></div>
