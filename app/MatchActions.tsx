...
export default function MatchActions({ id, stage, recruiterNote, nextAction }: { id:string; stage:string; recruiterNote:string|null; nextAction:string|null }) {
  ...
  async function save() {
    ...
    const res = await fetch(`/api/matches/${id}`, {
      ...
      body: JSON.stringify({ stage: currentStage, recruiterNote: note, nextAction: action })
    });
    setSaving(false);
  ...
  return <div className="opsPanel">
    <div className="field"><label>Étape du pipeline<select value={currentStage} onChange={(e)=>setCurrentStage(e.target.value)}>{PIPELINE_STAGES.map((s)=><option key={s.value} value={s.value}>{s.label}</option>)}</select></label></div>
