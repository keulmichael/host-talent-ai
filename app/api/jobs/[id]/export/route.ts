...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const job=await prisma.job.findFirst({where:{id,organizationId:user.organizationId},include:{matches:{where:{organizationId:user.organizationId},include:{candidate:true,feedbacks:{orderBy:{createdAt:"desc"}}},orderBy:{score:"desc"}}}});
 if(!job)return NextResponse.json({error:"Mission introuvable"},{status:404});
 const payload={
  schema:"host-talent-ai/job-export/v1.9",
  ...
  candidates:job.matches.map(m=>({
   candidateId:m.candidate.id,fullName:m.candidate.fullName,email:m.candidate.email,location:m.candidate.location,experienceYears:m.candidate.experienceYears,skills:m.candidate.skills.split(",").map(x=>x.trim()).filter(Boolean),availability:m.candidate.availability,dailyRate:m.candidate.dailyRate,salaryExpectation:m.candidate.salaryExpectation,
   ...
   clientFeedback:m.feedbacks.map(f=>({decision:f.decision,clientName:f.clientName,comment:f.comment,createdAt:f.createdAt}))
  }))
 };
