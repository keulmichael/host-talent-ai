...
export default async function Dashboard(){
 const[jobCount,candidateCount,matchCount,recentJobs,topMatches,activePipeline,strong]=await Promise.all([
  prisma.job.count(),prisma.candidate.count(),prisma.match.count(),
  ...
  prisma.match.findMany({where:{stage:{not:"NEW"}},take:6,orderBy:{updatedAt:"desc"},include:{candidate:true,job:true}}),
  prisma.match.count({where:{score:{gte:70}}})
 ]);
 return <>
  <div className="hero"><div><div className="eyebrow">COPILOTE RECRUTEUR · V1.5</div><h1>Du vivier au suivi opérationnel.</h1><p className="muted">Analyse des CV, matching explicable, recherche du vivier, short-list, prochaines actions et pipeline candidat.</p></div><div className="actions"><Link className="btn" href="/jobs/new">Créer une mission</Link><Link className="btn secondary" href="/search">Rechercher un talent</Link><Link className="btn secondary" href="/pipeline">Pipeline</Link><RecomputeAllButton/></div></div>
