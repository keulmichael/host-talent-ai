...
export default async function Dashboard(){
  ...
  const prequalifyCandidateCount=new Set(allMatches.filter(m=>["SHORTLIST","CONTACTED","INTERVIEW"].includes(m.stage)&&(!m.candidate.availability||(m.candidate.dailyRate==null&&m.candidate.salaryExpectation==null))).map(m=>m.candidateId)).size;
  ...
  return <>
    ...
    <section className="kpiRow">
      ...
      <div className="kpiCard"><span>À préqualifier</span><strong>{prequalifyCandidateCount}</strong><small>profils retenus : disponibilité et rémunération · <Link href="/prequalifications">ouvrir la file →</Link></small></div>
    ...
    <HorizontalBars title="Entonnoir de tri automatique" description="Le calcul est désormais dédupliqué par candidat sur son meilleur matching. La disponibilité et la rémunération ne sont demandées qu’aux profils que le recruteur a décidé de poursuivre." items={funnel}/>
    ...
    <section className="workflowStrip"><div><strong>Analyser</strong><span>Tous les matchings automatiquement</span></div><b>→</b><div><strong>Prioriser</strong><span>Meilleur matching par candidat</span></div><b>→</b><div><strong>Valider</strong><span>Le recruteur choisit les profils à poursuivre</span></div><b>→</b><Link href="/prequalifications"><strong>Préqualifier</strong><span>Disponibilité et rémunération des profils retenus</span></Link></section>
  </>;
}
