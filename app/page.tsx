...
export default async function Dashboard(){
 const user=await requireUser(); const organizationId=user.organizationId; const now=new Date(); const d30=new Date(now.getTime()+30*86400000);
 const[jobCount,candidateCount,matchCount,recentJobs,topMatches,activePipeline,strong,expiring,privateFiles,shortlisted]=await Promise.all([
 ...
 return <>
  <div className="hero"><div><div className="eyebrow">COPILOTE RECRUTEUR · V1.8</div><h1>Du vivier à la présentation client.</h1><p className="muted">{user.organization.name} · matching explicable, comparaison de profils, short-list client, export ATS/CRM et stockage privé des CV.</p></div><div className="actions"><Link className="btn" href="/jobs/new">Créer une mission</Link><Link className="btn secondary" href="/search">Recherche sémantique</Link><Link className="btn secondary" href="/pipeline">Pipeline</Link><RecomputeAllButton/></div></div>
  <div className="grid"><div className="card"><div className="muted">Missions</div><div className="score">{jobCount}</div></div><div className="card"><div className="muted">Candidats</div><div className="score">{candidateCount}</div></div><div className="card"><div className="muted">Adéquations ≥ 70</div><div className="score">{strong}</div></div><div className="card"><div className="muted">Profils en short-list / process</div><div className="score">{shortlisted}</div></div></div>
  {user.role==="ADMIN"&&<div className="grid" style={{marginTop:16}}><div className="card"><div className="muted">CV originaux stockés en privé</div><div className="score">{privateFiles}</div></div><div className="card"><div className="muted">Conservation à revoir ≤ 30 jours</div><div className="score">{expiring}</div><Link href="/admin/privacy" className="small">Ouvrir Confidentialité →</Link></div><div className="card"><div className="muted">Matchings calculés</div><div className="score">{matchCount}</div></div></div>}
  <div className="grid" style={{marginTop:16}}><div className="card"><div className="sectionHeader"><h2>Missions récentes</h2><Link href="/jobs">Tout voir →</Link></div>{recentJobs.length===0?<p className="muted">Aucune mission.</p>:recentJobs.map(j=>{const best=j.matches.length?Math.max(...j.matches.map(m=>m.score)):null;return <div className="listRow" key={j.id}><div><Link href={`/jobs/${j.id}`}><strong>{j.title}</strong></Link><div className="muted small"><Link href={`/jobs/${j.id}/compare`}>Comparer</Link> · <Link href={`/jobs/${j.id}/client`}>Dossier client</Link></div></div><span className="muted">{best==null?"Pas encore analysée":`meilleur score ${best}/100`}</span></div>})}</div>
