...
export default async function Dashboard(){
 ...
 const[jobCount,candidateCount,matchCount,recentJobs,topMatches,activePipeline,strong,expiring,privateFiles,shortlisted,activeShares,feedbackCount]=await Promise.all([
 ...
 return <>
  <div className="hero"><div><div className="eyebrow">COPILOTE RECRUTEUR · V1.9</div><h1>Du vivier au retour client.</h1><p className="muted">{user.organization.name} · matching explicable, short-list partageable, retours client, disponibilité/TJM et export ATS/CRM.</p></div><div className="actions"><Link className="btn" href="/jobs/new">Créer une mission</Link><Link className="btn secondary" href="/search">Recherche sémantique</Link><Link className="btn secondary" href="/pipeline">Pipeline</Link><RecomputeAllButton/></div></div>
  <div className="grid"><div className="card"><div className="muted">Missions</div><div className="score">{jobCount}</div></div><div className="card"><div className="muted">Candidats</div><div className="score">{candidateCount}</div></div><div className="card"><div className="muted">Profils en short-list / process</div><div className="score">{shortlisted}</div></div><div className="card"><div className="muted">Liens client actifs</div><div className="score">{activeShares}</div></div><div className="card"><div className="muted">Retours client reçus</div><div className="score">{feedbackCount}</div></div></div>
