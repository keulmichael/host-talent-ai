...
export default async function Dashboard(){
 ...
 const[jobCount,candidateCount,matchCount,recentJobs,topMatches,activePipeline,strong,expiring,privateFiles,shortlisted,activeShares,feedbackCount,dueActions,overdueActions,webhookCount,templateCount,surveyCount,surveyResponseCount]=await Promise.all([
  ...
  prisma.messageTemplate.count({where:{organizationId,active:true}}),
  prisma.candidateSurvey.count({where:{organizationId}}),
  prisma.candidateSurveyResponse.count({where:{survey:{organizationId}}})
 ]);
 return <>
  <div className="hero"><div><div className="eyebrow">COPILOTE RECRUTEUR · V2.4 · MODULES 01–04</div><h1>Du vivier à l'expérience candidat pilotée.</h1><p className="muted">{user.organization.name} · audit des frictions, matching explicable, orchestration relationnelle, SLA et observatoire de l'expérience candidat.</p></div><div className="actions"><Link className="btn" href="/jobs/new">Créer une mission</Link><Link className="btn secondary" href="/audit">Audit candidat</Link><Link className="btn secondary" href="/actions">Centre d'actions</Link><Link className="btn secondary" href="/experience">Observatoire candidat</Link><Link className="btn secondary" href="/search">Recherche sémantique</Link><Link className="btn secondary" href="/pipeline">Pipeline</Link><RecomputeAllButton/></div></div>
  <div className="grid"><div className="card"><div className="muted">Missions</div><div className="score">{jobCount}</div></div><div className="card"><div className="muted">Candidats</div><div className="score">{candidateCount}</div></div><div className="card"><div className="muted">Actions à venir ≤ 7 jours</div><div className="score">{dueActions}</div></div><div className="card"><div className="muted">Relances en retard</div><div className="score">{overdueActions}</div></div><div className="card"><div className="muted">Questionnaires candidat</div><div className="score">{surveyCount}</div></div><div className="card"><div className="muted">Retours candidat reçus</div><div className="score">{surveyResponseCount}</div></div><div className="card"><div className="muted">Profils en short-list / process</div><div className="score">{shortlisted}</div></div><div className="card"><div className="muted">Liens client actifs</div><div className="score">{activeShares}</div></div><div className="card"><div className="muted">Retours client reçus</div><div className="score">{feedbackCount}</div></div></div>
  {user.role==="ADMIN"&&<div className="grid" style={{marginTop:16}}><div className="card"><div className="muted">Connecteurs actifs</div><div className="score">{webhookCount}</div><Link href="/admin/automation" className="small">Configurer l'automatisation →</Link></div><div className="card"><div className="muted">Modèles relationnels actifs</div><div className="score">{templateCount}</div></div><div className="card"><div className="muted">CV originaux stockés en privé</div><div className="score">{privateFiles}</div></div><div className="card"><div className="muted">Conservation à revoir ≤ 30 jours</div><div className="score">{expiring}</div><Link href="/admin/privacy" className="small">Ouvrir Confidentialité →</Link></div><div className="card"><div className="muted">Adéquations ≥ 70</div><div className="score">{strong}</div></div><div className="card"><div className="muted">Matchings calculés</div><div className="score">{matchCount}</div></div></div>}
